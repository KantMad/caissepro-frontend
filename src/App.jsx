...
function AppProvider({children}){
  ...
  // CSV import
  const importCSV=useCallback((file)=>{Papa.parse(file,{header:true,skipEmptyLines:true,complete:(r)=>{
    const np=r.data.map((row,i)=>({id:String(Date.now()+i),name:row.name||row.nom||"Produit",sku:row.sku||`IMP-${i}`,
      price:parseFloat(row.price||row.prix||0),costPrice:parseFloat(row.costPrice||row.prix_achat||0),
      taxRate:parseFloat(row.taxRate||row.tva||0.20),category:row.category||row.categorie||"Divers",collection:row.collection||"",
      variants:[{id:`iv${Date.now()+i}`,color:row.color||row.couleur||"Défaut",size:row.size||row.taille||"TU",
        ean:row.ean||"",stock:parseInt(row.stock||0),defective:0,stockAlert:parseInt(row.stockAlert||5)}]}));
    setProducts(p=>[...p,...np]);addAudit("IMPORT",`${np.length} produits importés`);}});},[addAudit]);
  ...
  return<AppCtx.Provider value={{currentUser,login,logout,mode,setMode,offlineMode,products,setProducts,addProduct,customers,setCustomers,addCustomer,
    ...
    importCSV,findByEAN,perm,settings,setSettings,getLoyaltyTier,avoirPayment,setAvoirPayment,
...
function ProductsScreen(){
  const{products,setProducts,importCSV,addProduct,perm:p,exportCatalog,duplicateProduct,
    updateProduct,deleteProduct,addVariantToProduct,deleteVariant,updateProductPrice}=useApp();
  const[search,setSearch]=useState("");const[importModal,setImportModal]=useState(false);
  ...
  return(<div style={{height:"100%",overflowY:"auto",padding:20,background:C.bg}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
      ...
      <div style={{display:"flex",gap:6}}>
        <Btn variant="outline" onClick={()=>setImportModal(true)} style={{fontSize:11}}><Upload size={12}/> CSV</Btn>
    ...
    <Modal open={importModal} onClose={()=>setImportModal(false)} title="Import CSV">
      <input ref={fileRef} type="file" accept=".csv" style={{display:"none"}} onChange={e=>{if(e.target.files[0]){importCSV(e.target.files[0]);setImportModal(false);}}}/>
      <Btn onClick={()=>fileRef.current?.click()} style={{width:"100%",height:44,background:`linear-gradient(135deg,${C.primary},${C.gradientB})`}}><Upload size={16}/> Sélectionner CSV</Btn>
      <div style={{marginTop:8,fontSize:10,color:C.textMuted}}>Colonnes: name, sku, price, costPrice, taxRate, category, collection, color, size, ean, stock</div></Modal>
  </div>);
}
